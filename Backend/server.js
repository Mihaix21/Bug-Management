...
app.put('/projects/:projectId/bugs/:bugId/resolve', verifyRole('Member'), async (req, res) => {
  const { projectId, bugId } = req.params;
  const { status, resolutionCommitLink } = req.body;

  if (!status || !resolutionCommitLink) {
    return res.status(400).json({ message: "Invalid input. Both status and resolutionCommitLink are required." });
  }

  try {
    const bug = await Bug.findOneAndUpdate(
      { _id: bugId, projectId },
      { $set: { status, resolutionCommitLink } },
      { new: true }
    );
